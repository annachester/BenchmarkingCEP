...
$startflink
START=$(date +%s)
$flink run -c Q2_ANDQuery $jar --input $data_path3 --output $output_pathQ --vel 150 --qua 200 --wsize 20 --tput 500000
END=$(date +%s)
DIFF=$((END - START))
echo "Q2_ANDQuery run "$loop " select 1% : "$DIFF"s" >>$resultFile
$stopflink
...
$startflink
START=$(date +%s)
$flink run -c Q2_ANDPattern $jar --input $data_path4 --output $output_pathP --vel 150 --qua 200 --wsize 20 --tput 500000
END=$(date +%s)
DIFF=$((END - START))
echo "Q2_ANDPattern run "$loop " select 0.1% : "$DIFF"s" >>$resultFile
$stopflink
echo "------------ Flink stopped ------------" >>$resultFile
now=$(date +"%T")
...
$startflink
START=$(date +%s)
$flink run -c Q2_ANDQuery $jar --input $data_path4 --output $output_pathQ --vel 150 --qua 200 --wsize 20 --tput 500000
END=$(date +%s)
DIFF=$((END - START))
echo "Q2_ANDQuery run "$loop " select 0.1% : "$DIFF"s" >>$resultFile
$stopflink
echo "------------ Flink stopped ------------" >>$resultFile
now=$(date +"%T")
...
$startflink
START=$(date +%s)
$flink run -c Q2_ANDPattern $jar --input $data_path5 --output $output_pathP --vel 150 --qua 200 --wsize 20 --tput 500000
END=$(date +%s)
DIFF=$((END - START))
echo "Q2_ANDPattern run "$loop " select 10% skewed : "$DIFF"s" >>$resultFile
$stopflink
echo "------------ Flink stopped ------------" >>$resultFile
now=$(date +"%T")
...
$startflink
START=$(date +%s)
$flink run -c Q2_ANDQuery $jar --input $data_path5 --output $output_pathQ --vel 150 --qua 200 --wsize 20 --tput 500000
END=$(date +%s)
DIFF=$((END - START))
echo "Q2_ANDQuery run "$loop " select 10% skewed : "$DIFF"s" >>$resultFile
$stopflink
echo "------------ Flink stopped ------------" >>$resultFile
now=$(date +"%T")
