...
$startflink
START=$(date +%s)
$flink run -c Q3_ORPattern $jar --input $data_path1 --output $output_pathP --vel 150 --qua 200 --wsize 20 --tput 500000
END=$(date +%s)
DIFF=$((END - START))
echo "Q3_ORPattern run "$loop " select 10% : "$DIFF"s" >>$resultFile
$stopflink
...
$startflink
START=$(date +%s)
$flink run -c Q3_ORQuery $jar --input $data_path1 --output $output_pathQ --vel 150 --qua 200 --wsize 20 --tput 500000
END=$(date +%s)
DIFF=$((END - START))
echo "Q3_ORQuery run "$loop " select 10% : "$DIFF"s" >>$resultFile
$stopflink
echo "------------ Flink stopped ------------" >>$resultFile
now=$(date +"%T")
...
#$startflink
#START=$(date +%s)
#$flink run -c Q3_ORPattern $jar --input $data_path2 --output $output_pathP --vel 150 --qua 200 --wsize 20 --tput 500000
#END=$(date +%s)
#DIFF=$((END - START))
#echo "Q3_ORPattern run "$loop " select 200% : "$DIFF"s" >>$resultFile
#$stopflink
#echo "------------ Flink stopped ------------" >>$resultFile
#now=$(date +"%T")
...
#$startflink
#START=$(date +%s)
#$flink run -c Q3_ORQuery $jar --input $data_path2 --output $output_pathQ --vel 150 --qua 200 --wsize 20 --tput 500000
#END=$(date +%s)
#DIFF=$((END - START))
#echo "Q3_ORQuery run "$loop " select 200% : "$DIFF"s" >>$resultFile
#$stopflink
#echo "------------ Flink stopped ------------" >>$resultFile
#now=$(date +"%T")
...
$startflink
START=$(date +%s)
$flink run -c Q3_ORPattern $jar --input $data_path3 --output $output_pathP --vel 150 --qua 200 --wsize 20 --tput 500000
END=$(date +%s)
DIFF=$((END - START))
echo "Q3_ORPattern run "$loop " select 1% : "$DIFF"s" >>$resultFile
$stopflink
echo "------------ Flink stopped ------------" >>$resultFile
now=$(date +"%T")
