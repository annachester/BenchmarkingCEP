...
// relaxed contiguity
public class Q8_2_SEQPatternLS {
    public static void main(String[] args) throws Exception {

        String className = "Q8_2_SEQPatternLS";
        ...
        Integer quaFilter = parameters.getInt("qua",200);
...
//        DataStream<String> throughput_messages = input.flatMap(new ThroughputLogger<KeyedDataPointGeneral>(KeyedDataPointSourceFunction.RECORD_SIZE_IN_BYTE, throughput));
      DataStream<String> throughput_messages = input.flatMap(new ThroughputLogger<KeyedDataPointGeneral>(KeyedDataPointSourceFunction.RECORD_SIZE_IN_BYTE,
        ...
        Pattern<KeyedDataPointGeneral, ?> pattern = Pattern.<KeyedDataPointGeneral>begin("first").subtype(
                ...
                }).next("second").subtype(QuantityEvent.class).where(
            new IterativeCondition<QuantityEvent>() {
                ...
                }).next("third").subtype(VelocityEvent.class).where(
            new IterativeCondition<VelocityEvent>() {
              @Override
              public boolean filter(VelocityEvent event3, Context<VelocityEvent> ctx)
                  throws Exception {
                Double velocity = (Double) event3.getValue();
                if (velocity > velFilter){ //velocity increase
                  for (KeyedDataPointGeneral event1 : ctx.getEventsForPattern("first")) {
                    return (Double) event3.getValue() > (Double) event1.getValue();
                  }
                }
                return false;
              }
            }).next("last").subtype(QuantityEvent.class).where(
                new IterativeCondition<QuantityEvent>() {
                    @Override
                    public boolean filter(QuantityEvent event4, Context<QuantityEvent> ctx) throws Exception {
                        Double quantity = (Double) event4.getValue();
                    if (quantity < quaFilter){ //quantity decrease
                        for (KeyedDataPointGeneral event2 : ctx.getEventsForPattern("second")) {
                            boolean match = (Double) event4.getValue() < (Double) event2.getValue();
                            if (match) {
                                // moment the last event of the sequence is detected
                                event4.setDetectionTimeStampMs(System.currentTimeMillis());
                            }
                            return match;
                        }
                    }
