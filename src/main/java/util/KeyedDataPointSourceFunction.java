...
public class KeyedDataPointSourceFunction implements SourceFunction<KeyedDataPointGeneral> {
    ...
    public KeyedDataPointSourceFunction(String fileName, String delimiter, long throughput) {
     ...
     public KeyedDataPointSourceFunction(String fileName, Integer file_loops, String delimiter) {
        this.file = fileName;
        this.key = null;
    ...
    public KeyedDataPointSourceFunction(String fileName, Integer file_loops, String delimiter, long throughput) {
        this.file = fileName;
        this.key = null;
        this.sourcefile_loops = file_loops;
        this.delimiter = delimiter;
    ...
    public KeyedDataPointSourceFunction(String fileName, Integer file_loops, String key, String delimiter) {
        this.file = fileName;
        this.key = key;
        this.sourcefile_loops = file_loops;
        this.delimiter = delimiter;
    ...
    public KeyedDataPointSourceFunction(String fileName, Integer file_loops, String key, String delimiter, long throughput) {
        this.file = fileName;
        this.key = key;
        this.sourcefile_loops = file_loops;
        this.delimiter = delimiter;
     ...
     public void run(SourceContext<KeyedDataPointGeneral> sourceContext) throws Exception {
        ...
        try {
            ...
            while (scan.hasNext()) {
                ...
                String id;
                if (this.key == null || ((this.key.equals("1") && this.sourcefile_loops == 1) && !data[0].contains("_id"))) id = data[0].trim();
                else id = this.key;
                ...
                if (data.length == 6) {
                    ...
                    // parse QnV
                    // TODO: not true.. QnV has length 6.. is parsed by previous if
                    // time
                    if (this.sourcefile_loops == 1 || loopCount == 1 ) {
                ...
                // A: sets currentTime after finishing every loop of the file
                // and creates new scanner so while loop continues
                ...
                // A: if specified data ingestion rate per second is reached (throughput), sleep
                // (manipulateIngestionRate only false when tthroughput was set to 0 at Init)
                if(tupleCounter >= throughput && manipulateIngestionRate){
                    long now = System.currentTimeMillis();
                    if ((1000-(now-start)) > 0) { // if less than 1 second passed, sleep
                        Thread.sleep(1000 - (now - start));
//                        System.out.println("sleep");
                    }
                    else{
                        //Log.info("Throughput is already lower than " + this.throughput + "per second.");
//                        System.out.println("Throughput is already lower than " + this.throughput + "per second.");
                    }
                    tupleCounter = 0;
